...
int main(int argc, char const* argv[]){
    for(int i=1; i<31; i++){
        ...
        char string[100]="testsNG_medium/test_";
        char str[3];
        sprintf(str, "%d", i);
        strcat(string, str);
        strcat(string, ".txt");
...
//one th per no good
__global__ void removeNoGoodSetsContaining(int* matrix, int* returningNGchanged, int* dev_partialAssignment, int* dev_matrix_noGoodsStatus, int* dev_currentNoGoods,int * SM_dev_currentNoGoods,int* dev_decreaseNoGoods) {

    int thPos = blockIdx.x * blockDim.x + threadIdx.x;

    extern __shared__ int decrease[]; //has block size
    __shared__ int s_returningNGchanged[194];
    ...
    dev_decreaseNoGoods[thPos]=decrease[threadIdx.x];
...
__global__ void removeLiteralFromNoGoods(int* dev_matrix, int* dev_currentNoGoods,int* dev_noOfVarPerNoGood, int* dev_lonelyVar, int* partialAssignment,int* SM_dev_conflict) {
    ...
    __shared__ int s_dev_currentNoGoods[193];
